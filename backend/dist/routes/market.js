...
router.get('/overview', async (req, res) => {
    try {
        ...
        const marketOverview = await stockData_1.StockDataService.getMarketOverview();
        res.json({
            success: true,
            data: {
        ...
        logger_1.logger.error('Market overview failed:', error);
        res.status(500).json({
...
router.get('/indices', async (req, res) => {
    try {
        const indices = await database_1.DatabaseService.getMarketIndices();
        res.json({
            success: true,
        ...
        logger_1.logger.error('Failed to get market indices:', error);
        res.status(500).json({
...
router.get('/movers', [
    ...
    try {
        ...
        const moversData = await getMoversData(type, Number(limit));
        res.json({
            success: true,
            data: {
                type,
        ...
        logger_1.logger.error('Failed to get movers:', error);
        res.status(500).json({
...
router.get('/trending', [
    (0, express_validator_1.query)('period').optional().isIn(['1h', '4h', '1d', '1w']).default('1d'),
    (0, express_validator_1.query)('limit').optional().isInt({ min: 1, max: 50 }).default(20),
], validateRequest, async (req, res) => {
    try {
        const { period, limit } = req.query;
        const trendingStocks = await getTrendingStocks(period, Number(limit));
        res.json({
            success: true,
            data: {
                period,
        ...
        logger_1.logger.error('Failed to get trending stocks:', error);
        res.status(500).json({
...
router.get('/news', [
    ...
    try {
        ...
        const newsData = await getMarketNews(category, Number(limit));
        res.json({
            success: true,
            data: {
                category,
...
router.get('/economic-indicators', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Failed to get economic indicators:', error);
        res.status(500).json({
...
router.get('/volatility', [
    (0, express_validator_1.query)('period').optional().isIn(['1d', '1w', '1m', '3m']).default('1m'),
], validateRequest, async (req, res) => {
    try {
        const { period } = req.query;
        const volatilityData = await getVolatilityData(period);
        res.json({
            success: true,
            data: {
                period,
        ...
        logger_1.logger.error('Failed to get volatility data:', error);
        res.status(500).json({
...
router.get('/sentiment', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Failed to get market sentiment:', error);
        res.status(500).json({
...
router.get('/calendar', [
    ...
    (0, express_validator_1.query)('type').optional().isIn(['earnings', 'dividends', 'splits', 'ipos', 'all']).default('all'),
], validateRequest, async (req, res) => {
    try {
        const { start_date, end_date, type } = req.query;
        const calendarData = await getMarketCalendar(start_date, end_date, type);
        res.json({
            success: true,
            data: {
                events: calendarData,
        ...
        logger_1.logger.error('Failed to get market calendar:', error);
        res.status(500).json({
            error: 'Failed to get market calendar',
        });
    }
});
async function getMoversData(type, limit) {
    const mockData = {
        gainers: [
...
async function getTrendingStocks(period, limit) {
...
async function getVolatilityData(period) {
    return {
        vix: {
            current: 18.5,
            change: -1.2,
...
async function getMarketCalendar(startDate, endDate, type) {
    const events = [
        {
            id: '1',
            type: 'earnings',
